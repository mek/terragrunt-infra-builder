...
sub create_resource {
    ...
    my $resource_type =
      $args{resource_type} || extract_resource_type($resource_name);
    ...
    if ( !$module ) {
        ...
        $args{resource_type} =
          $resource_type;    # Pass the intended type to generic
    }
...
sub list_resource_types {
    ...
    if ( -d $resources_dir ) {
        opendir( my $dh, $resources_dir )
          or die "Cannot open $resources_dir: $!";
...
sub get_resource_types_by_category {
    my %categories = (
        ...
        storage => [
            's3', 's3-backend', 'external-s3-backend', 'efs', 'fsx', 'ebs',
            'backup'
        ],
        database => [
            'rds',          'rds-mysql',
        ...
        security => [
            ...
            'iam-group', 'kms',      'secrets-manager', 'acm',
