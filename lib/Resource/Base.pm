...
sub new {
    ...
    $self->{workspace_root} = $args{workspace_root}
      or die "Workspace root required";
    $self->{template_dir} = $args{template_dir}
      or die "Template directory required";

    # Parameters with defaults
    $self->{envs_base}     = $args{envs_base}     || "envs";
    $self->{projects_base} = $args{projects_base} || "projects";
    $self->{structure_ordering} =
      $args{structure_ordering} || "environment_first";
    ...
    if ( $ENV{DEBUG} ) {
        print "    After separate assignment: self->{region_name} = "
          . (
            defined( $self->{region_name} )
            ? "'$self->{region_name}'"
            : 'undef' ) . "\n";
    }
    ...
    if ( $ENV{DEBUG} ) {
        ...
        print "      region_name => "
          . ( defined( $self->{region_name} ) ? $self->{region_name} : 'undef' )
          . "\n";
    }
...
sub get_resource_path {
        ...
        || $structure_order eq "project-first" )
    {
        ...
        die "project_name required for project_first structure"
          unless $self->{project_name};

        $path =
"$self->{workspace_root}/$self->{projects_base}/$self->{project_name}/$self->{env_name}";
    }
    else {
...
sub get_dependency_path {
    ...
    if ( defined $dep_location ) {
        if ( $dep_location eq 'env' ) {
            ...
            $levels_up =
              $self->{zone_name} ? 3 : ( $self->{region_name} ? 2 : 1 );
        }
...
sub process_template {
    ...
    # Read template
    open( my $in_fh, '<', $template_file )
      or die "Cannot read $template_file: $!";
...
sub create {
    ...
    if ( -d $resource_path && !$self->{force} && !$self->{dry_run} ) {
        die
          "Resource $self->{name} already exists. Use -f to force overwrite.\n";
    }
...
sub generate_inputs_json {
    ...
    my $analyzer_script =
      "$self->{workspace_root}/admin/terraform-module-analyzer.pl";
    ...
    eval {
        ...
        if ( $? != 0 ) {
            ...
            if ( -f $inputs_file ) {
                ...
                print
"  ℹ Module analyzer completed, but no inputs.json generated for this resource\n"
                  if $self->{verbose};
            }
        }
    };
