...
sub load_configuration {
    ...
    if ( -f $self->{file} ) {
        eval {
            ...
            $self->{data} =
              $self->{data}->[0];    # YAML::Tiny returns array reference
        };
        if ($@) {
            warn
"${YELLOW}Warning: Error loading configuration file $self->{file}: $@${NC}\n";
        ...
        $self->set_default_config();
        print
"${YELLOW}Using default configuration (no config file found at $self->{file})${NC}\n"
          if $self->{verbose};
    }
}
...
sub set_instance {
    ...
    if ( defined $new_instance && !$new_instance->isa(__PACKAGE__) ) {
        croak "Argument to set_instance() must be an instance of "
          . __PACKAGE__;
    }
