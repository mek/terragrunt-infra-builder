...
sub validate {
    ...
    if ($self->{name} !~ /^[a-z0-9-]+$/) {
        die "Project name must contain only lowercase letters, numbers, and hyphens\n";
    }
...
sub post_create {
    ...
    print "$Infrastructure::Base::GREEN✓ Project $self->{name} created successfully$Infrastructure::Base::NC\n";
}
