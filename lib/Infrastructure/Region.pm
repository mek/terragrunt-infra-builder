...
"Region name must contain only lowercase letters, numbers, and hyphens\n";
    ...
    if ( $self->{name} !~ /^(us|eu|ap|ca|sa|af|me)-[a-z]+-\d+$/ ) {
        print
"  Warning: Region name doesn't follow AWS format (us-east-1, eu-west-1, etc.)\n"
          if $self->{verbose};
    }
    ...
    if ( !-d $env_path ) {
        die
          "Environment '$self->{env_name}' does not exist. Create it first.\n";
    }
...
sub post_create {
    ...
    if ( $self->{verbose} ) {
        print
"  Region '$self->{name}' created for environment '$self->{env_name}'\n";
        if ( $self->{project_name} ) {
    ...
    print
"$Infrastructure::Base::GREEN✓ Region $self->{name} created successfully$Infrastructure::Base::NC\n";
}
