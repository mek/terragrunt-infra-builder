...
sub validate {
    ...
    if ( $region_part ne $self->{region_name} ) {
        die
"Zone name '$self->{name}' doesn't match region '$self->{region_name}'\n";
    }
...
sub post_create {
    ...
    if ( $self->{verbose} ) {
        print
          "  Zone '$self->{name}' created in region '$self->{region_name}'\n";
    ...
    print
"$Infrastructure::Base::GREEN✓ Zone $self->{name} created successfully$Infrastructure::Base::NC\n";
}
