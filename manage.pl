...
sub main {
    ...
    $template_dir =
      "$workspace_root/" . get_config_value("templates.base_path");
...
sub validate_workspace_structure {
    my $envs_dir =
      "$workspace_root/" . get_config_value("directories.environments");
    if ( !-d $envs_dir ) {
        warn
          "${YELLOW}Warning: Environments directory not found: $envs_dir${NC}\n"
          if $verbose;
    }
}
...
sub detect_workspace_structure {
    ...
    my $has_envs      = -d "$workspace_root/$envs_base"
      && directory_has_content("$workspace_root/$envs_base");
    my $has_projects = -d "$workspace_root/$projects_base"
      && directory_has_content("$workspace_root/$projects_base");

    my $detected_order;
    if ( $has_envs && !$has_projects ) {
        $detected_order = "environment_first";
        print
"${CYAN}Auto-detected structure: environment_first ($envs_base/ directory found)${NC}\n"
          if $verbose;
    }
    elsif ( $has_projects && !$has_envs ) {
        $detected_order = "project_first";
        print
"${CYAN}Auto-detected structure: project_first ($projects_base/ directory found)${NC}\n"
          if $verbose;
    }
        ...
        $detected_order =
             $structure_order
          || get_config_value("directories.structure_ordering")
          || "environment_first";
        ...
        die
"${RED}ERROR: Both '$envs_base/' and '$projects_base/' directories exist with content. Please clean up or manually set structure lock.${NC}\n";
    }
...
sub read_structure_lock {
    ...
    open my $fh, '<', $lock_file
      or die "Cannot read structure lock file $lock_file: $!";
...
sub write_structure_lock {
    ...
    my $lock_data = {
        ...
        detected_patterns  => {
              has_envs_dir => ( -d "$workspace_root/$envs_base" ) ? JSON::true
            : JSON::false,
            has_projects_dir => ( -d "$workspace_root/$projects_base" )
            ? JSON::true
            : JSON::false,
        }
    };
    ...
    open my $fh, '>', $lock_file
      or die "Cannot write structure lock file $lock_file: $!";
...
sub validate_structure_consistency {
    ...
    if ( $requested_order && $normalized_requested ne $normalized_current ) {
        ...
        print
          "  Current workspace structure: ${YELLOW}$current_structure${NC}\n";
        ...
        if ( !$force ) {
            print "To fix this:\n";
            print
              "  1. Remove --structure-order flag to use current structure\n";
            print
              "  2. Or use --force to override (may cause inconsistencies)\n";
            print
              "  3. Or migrate structure using: manage.pl migrate-structure\n";
            exit 1;
        }
        else {
            print
"${YELLOW}WARNING: Forcing structure override. This may cause inconsistencies.${NC}\n";
        }
    }
...
sub parse_resource_specification {
    ...
    if ( $resource_spec =~ /^([^:]+):(.+)$/ ) {
        ...
        if ( exists $supported_types{$type} ) {
            ...
            print
"${CYAN}Creating resource '$name' of type '$canonical_type'${NC}\n"
              if $verbose;
...
sub execute_add {
    ...
    if ( $target eq 'env' ) {
        ...
        die "Environment (-e) required for regional resources"
          unless $options{env};
        add_regional_resources( $region_name, $options{env},
            $options{project} );
    }
        ...
        die "Environment (-e) and region (-r) required for zone"
          unless $options{env} && $options{region};
        add_zone( $zone_name, $options{env}, $options{region},
            $options{project} );
    }
        ...
        my ( $resource_name, $resource_type ) =
          parse_resource_specification($resource_spec);
...
sub add_regional_resources {
    ...
    if ($project_name) {
        print
"${CYAN}Adding regional resources: $region_name for environment: $env_name, project: $project_name${NC}\n";
    }
    else {
        print
"${CYAN}Adding regional resources: $region_name for environment: $env_name${NC}\n";
    }
...
sub add_zone {
    ...
    if ( !$allowed_zones || !exists $allowed_zones->{$zone_name} ) {
        ...
        print
"${RED}Error: Zone '$zone_name' is not in the list of allowed zones${NC}\n";
        if (@valid_zones) {
    ...
    print
"${CYAN}Adding zone: $zone_name ($zone_short_name) in region: $region_name for environment: $env_name${NC}\n";
...
sub add_resource {
    ...
    if (
        !$options{project}
        && get_config_value(
            "directories.environment_structure.regional_placement") eq
        "project_based"
      )
    {
        $options{project} =
          get_config_value("directories.environment_structure.default_project");
    }
...
sub create_resource_terragrunt {
    my (
        $resource_dir, $resource_name, $resource_type, $env_name,
        $region_name,  $zone_name,     $hierarchy_level
    ) = @_;
    ...
    if ( !$dry_run ) {
        open( my $fh, '>', $terragrunt_file )
          or die "Cannot write $terragrunt_file: $!";
        print $fh $content;
        close $fh;
...
sub generate_terragrunt_header {
    ...
    if ( $context->{zone_name} && $context->{allowed_zones} ) {
        my $zone_short =
          $context->{allowed_zones}->{ $context->{zone_name} } || "unknown";
    ...
    return join(
        ...
        "# Zone: "
          . (
            $context->{zone_name} ? "$context->{zone_name}$zone_info" : "N/A"
          ),
...
'  region_vars = read_terragrunt_config(find_in_parent_folders("region.hcl"))';
    ...
    push @locals, ( '  ', '  environment = local.env_vars.locals.environment' );
    ...
    if ( $context->{zone_name} ) {
        ...
        if (   $context->{allowed_zones}
            && $context->{allowed_zones}->{ $context->{zone_name} } )
        {
            push @locals,
"  zone_short = \"$context->{allowed_zones}->{$context->{zone_name}}\"";
        }
    }
...
sub generate_terragrunt_inputs {
    ...
    if ( $context->{zone_name} ) {
        ...
        {
            push @inputs,
"    ZoneShort   = \"$context->{allowed_zones}->{$context->{zone_name}}\"";
        }
    }
...
sub update_environment_files {
    ...
    update_file_content( "$env_dir/terragrunt.hcl", 'env_name', $env_name )
      if -f "$env_dir/terragrunt.hcl";
}
...
sub update_region_files {
    ...
    update_file_content( "$region_dir/terragrunt.hcl", 'region_name',
        $region_name )
      if -f "$region_dir/terragrunt.hcl";
    update_file_content( "$region_dir/terragrunt.hcl", 'env_name', $env_name )
      if -f "$region_dir/terragrunt.hcl";
}
...
sub update_zone_files {
    ...
    update_file_content( "$zone_dir/terragrunt.hcl", 'zone_name', $zone_name )
      if -f "$zone_dir/terragrunt.hcl";
    update_file_content( "$zone_dir/terragrunt.hcl", 'region_name',
        $region_name )
      if -f "$zone_dir/terragrunt.hcl";
    update_file_content( "$zone_dir/terragrunt.hcl", 'env_name', $env_name )
      if -f "$zone_dir/terragrunt.hcl";
}
...
sub execute_list {
    ...
    if ( $target eq 'env' ) {
        ...
        die "Environment (-e) and region (-r) required"
          unless $options{env} && $options{region};
...
sub list_regions {
    ...
    my $regional_placement =
      get_config_value("directories.environment_structure.regional_placement");
    my $global_dir_name =
      get_config_value("directories.environment_structure.global")
      || '__global';
    ...
    if ( $regional_placement eq "project_based" ) {
        ...
        foreach my $project_dir (@projects) {
            ...
            @project_regions =
              grep { basename($_) ne $global_dir_name } @project_regions;
        ...
        @direct_regions =
          grep { basename($_) ne $global_dir_name } @direct_regions;
...
sub list_zones {
    ...
    print
"${CYAN}Zones in region '$region_name' of environment '$env_name':${NC}\n";

    my $envs_base = get_config_value("directories.environments");
    my $regional_placement =
      get_config_value("directories.environment_structure.regional_placement");
    my $regional_dir_name =
      get_config_value("directories.environment_structure.regional");
    ...
    if ( $regional_placement eq "project_based" ) {
        ...
        if ( $options{project} ) {
            ...
            $region_dir =
"$workspace_root/$envs_base/$env_name/$options{project}/$region_name";
        }
        else {
            ...
            if ( $region_name =~ /(.+)\/(.+)/ ) {
                ...
                $region_dir =
                  "$workspace_root/$envs_base/$env_name/$project/$region";
            }
            else {
            # No project specified and no / in region name, can't determine path
                print
"  ${RED}Error: Project (-p) required for project-based regional placement${NC}\n";
                return;
            }
        }
    }
...
sub list_available_zones {
    ...
    if (%$allowed_zones) {
        ...
        foreach my $zone ( keys %$allowed_zones ) {
            $max_zone_length = length($zone)
              if length($zone) > $max_zone_length;
        }
...
sub execute_show {
    ...
    print
      "This would show detailed information about infrastructure components.\n";
}
...
sub execute_add_from_config {
    ...
    print
"${CYAN}Adding infrastructure components from config file: $config_file${NC}\n";
...
# Process global resources (deprecated - use environment-level resources instead)
    if ( $config->{global_resources} ) {
        warn
"${YELLOW}Warning: global_resources is deprecated. Use environment-level resources instead.${NC}\n";
...
# Skip deprecated functionality - count as successful for backward compatibility
    ...
    if ( $config->{regions} ) {
        foreach my $region_config ( @{ $config->{regions} } ) {
            ...
            my $project_name =
              $region_config->{project};    # Optional project from config
            print
"\n${BLUE}Processing region: $region_name for environment: $env_name${NC}\n";
            if (
                add_regional_resources(
                    $region_name, $env_name, $project_name
                )
              )
            {
                $successful_components++;
...
sub read_json_config {
    ...
    # Parse JSON
    my $config;
    eval { $config = JSON->new->decode($content); };
    if ($@) {
...
sub validate_config {
    ...
    if ( $config->{global_resources}
        && ref( $config->{global_resources} ) ne 'ARRAY' )
    {
        die
"${RED}Global resources must be an array (deprecated - use environment-level resources instead)${NC}\n";
    }
    ...
    if ( $config->{regions} ) {
        ...
        foreach my $region ( @{ $config->{regions} } ) {
            if ( !defined $region->{name} || !defined $region->{environment} ) {
                die
"${RED}Each region must have 'name' and 'environment' fields${NC}\n";
            }
        }
    }
    ...
    if ( $config->{zones} ) {
        ...
        foreach my $zone ( @{ $config->{zones} } ) {
            if (   !defined $zone->{name}
                || !defined $zone->{environment}
                || !defined $zone->{region} )
            {
                die
"${RED}Each zone must have 'name', 'environment', and 'region' fields${NC}\n";
            }
        }
    }
