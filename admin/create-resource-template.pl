...
sub main {
    ...
    if ( !exists $supported_types{$resource_type} ) {
        ...
        my @suggestions =
          Resource::Factory::suggest_resource_types($resource_type);
        if (@suggestions) {
...
sub create_terragrunt_template {
    ...
    my $content =
      generate_terragrunt_template_content( $resource_type, $category,
        $terraform_module );
...
sub create_resource_module {
    ...
    my $content =
      generate_resource_module_content( $resource_type, $module_name );
...
sub generate_readme_content {
    ...
    my $category  = get_resource_category( $resource_type,
        Resource::Factory::get_resource_types_by_category() );
...
sub generate_inputs_json_template {
    ...
    my $module_source =
      extract_module_source_from_template("$template_path/terragrunt.hcl");

    if ( !$module_source ) {
        print
"    ${YELLOW}Warning: Could not extract module source, creating basic inputs.json${NC}\n";
    ...
    if ( !$variables || !@$variables ) {
        print
"    ${YELLOW}Warning: Could not analyze module, creating basic inputs.json${NC}\n";
        create_basic_inputs_json( $template_path, $resource_type );
        return;
    }

    # Create inputs.json with actual module variables
    create_inputs_json_from_variables( $template_path, $resource_type,
        $variables, $module_source );

    print "    ✓ inputs.json generated with "
      . scalar(@$variables)
      . " variables from module analysis\n";
}
...
sub clone_and_analyze_terraform_module {
    ...
    # Find variables.tf
    my $variables_file =
      find_terraform_variables_file( $repo_dir, $module_info->{module_path} );
    my $variables = [];
...
# Handle Terraform Registry format: tfr:///terraform-aws-modules/vpc/aws?version=5.5.1
    if ( $source =~ /^tfr:\/\/\/(.+?)\/(.+?)\/(.+?)(?:\?(.+))?$/ ) {
        ...
        return {
            ...
            repo =>
              "https://github.com/$namespace/terraform-$provider-$name.git",
            module_path => '',
            ref         => $version
        };
    }
    ...
    foreach my $var (@$variables) {
        my $var_entry = {
            description => $var->{description}
              || "Variable from Terraform module",
        ...
        if ( $var->{name} =~ /^(name|identifier)$/ ) {
            $var_entry->{template_suggestion} =
              "Use {{full_name}} for consistent naming";
        }
        elsif ( $var->{name} eq 'tags' ) {
            $var_entry->{template_suggestion} =
              "Standard tags are added in terragrunt.hcl";
        }
        elsif ( $var->{name} =~ /^(vpc_id|subnet_ids?|security_group_ids?)$/ ) {
            $var_entry->{template_suggestion} =
              "Reference from dependency block in terragrunt.hcl";
        }
...
sub create_basic_inputs_json {
    ...
    my $inputs = {
        '_metadata' => {
            ...
            note                      =>
'Basic template - run terraform-module-analyzer.pl for detailed analysis'
        },
        'template_variables' => {
            resource_name => '{{resource_name}}',
            full_name     => '{{full_name}}',
...
sub print_next_steps {
    ...
    print
      "2. ${GREEN}inputs.json has been generated from module analysis${NC}\n";
    print "3. Test resource creation:\n";
    print
"   ${CYAN}./manage.pl add resource \"$resource_type\" -e dev -r us-west-2 --dry-run${NC}\n";
    print "4. Deploy when ready:\n";
    print
"   ${CYAN}./manage.pl add resource \"$resource_type\" -e dev -r us-west-2${NC}\n";
    print
