...
sub analyze_unique_modules {
    ...
    foreach my $type (keys %grouped_modules) {
        print "    $type: " . scalar(@{$grouped_modules{$type}}) . " modules\n";
        if ($type eq 'skip') {
...
sub convert_terraform_type_to_json_schema {
    ...
    if ($terraform_type =~ /^string$/) {
        ...
        return {
            type => 'object',
            additionalProperties => convert_terraform_type_to_json_schema($item_type)
        };
    }
        ...
        if ($schema) {
            ...
            foreach my $var_name (keys %{$schema->{properties}}) {
                ...
                my $var_info = {
                    ...
                    required => (grep { $_ eq $var_name } @{$schema->{required}}) ? 1 : 0,
...
sub save_analysis_results {
    ...
    my $results = {
        summary => {
            ...
            total_references => scalar(map { @{$modules{$_}} } keys %modules),
    ...
    if (!$dry_run) {
        open(my $fh, '>', $schemas_file) or die "Cannot write $schemas_file: $!";
