...
sub discover_modules {
    ...
    my @find_cmd = (
        'find', $envs_dir, '-name', 'terragrunt.hcl', '-not', '-path',
        '*/.terragrunt-cache/*'
    );
    ...
    while ( my $file = <$fh> ) {
        ...
        print " [has deploy.pl]"
          if $verbose && $modules{$module_path}{has_config};
        print "\n" if $verbose;
    }
    ...
    print
      "${GREEN}Discovered $count Terragrunt modules in '$envs_dir'${NC}\n\n";
}
                ...
                if ( $dep !~ /^\// ) {
                    ...
                    if ( exists $modules{$dep} ) {
                        ...
                        if ( @matches == 1 ) {
                            ...
                            warn
"${YELLOW}Warning: Multiple modules named '$dep', using path instead${NC}\n";
                            next;
                        }
                        else {
                            warn
"${YELLOW}Warning: Dependency '$dep' not found for $module_path${NC}\n";
                            next;
                        }
                    }
                }
...
sub parse_terragrunt_dependencies {
    ...
    while (
        $content =~ /dependency\s+"([^"]+)"\s*{[^}]*path\s*=\s*"([^"]+)"/gs )
    {
        ...
        if ( $dep_path =~ /^\.\./ ) {
            ...
            if ( exists $modules{$abs_dep_path} ) {
                ...
                unless ( grep { $_ eq $abs_dep_path }
                    @{ $modules{$module_path}{dependencies} } )
                {
                    push @{ $modules{$module_path}{dependencies} },
                      $abs_dep_path;
                    push @{ $modules{$abs_dep_path}{dependents} }, $module_path;
                    print
                      "  $module_path -> $abs_dep_path (from terragrunt.hcl)\n"
                      if $verbose;
                }
            }
        }
...
sub calculate_deployment_order {
    ...
    if ( scalar @deployment_order != scalar keys %modules ) {
        my @stuck = grep {
            !grep { $_ eq $modules{$_} } @deployment_order
        } keys %modules;
        die "${RED}Error: Circular dependencies detected involving: "
          . join( ", ", @stuck )
          . "${NC}\n";
    }
...
sub apply_filters {
    ...
    if ($directory_filter) {
        ...
        print "${YELLOW}Filtered to "
          . scalar(@filtered_order)
          . " modules in $directory_filter${NC}\n";
    }
    ...
    if ($tag_filter) {
        ...
        @filtered_order = grep {
            my $module = $modules{$_}{module};
              ...
              && ( !defined $tag_value
                || $module->tags->{$tag_key} eq $tag_value )
        } @filtered_order;
        print "${YELLOW}Filtered to "
          . scalar(@filtered_order)
          . " modules with tag $tag_filter${NC}\n";
    }
    ...
    if ( $output_format eq 'json' ) {
        ...
        foreach my $module_path (@deployment_order) {
            ...
            if ( $module->tags && %{ $module->tags } ) {
                print " ${CYAN}[";
                print join( ', ',
                    map { "$_=$module->{tags}{$_}" } keys %{ $module->tags } );
                print "]${NC}";
            }
...
sub execute_deployment {
    if ($dry_run) {
        print
"${YELLOW}DRY RUN MODE - No actual deployment will be performed${NC}\n\n";
    }
    ...
    foreach my $module_path (@deployment_order) {
        ...
        if ( $module->should_skip($context) ) {
            print
              "${BLUE}[$current/$total] ${YELLOW}Skipping: $module_path${NC}\n";
        ...
        if ( !$dry_run ) {
            ...
            while ( $attempts < $max_attempts && !$success ) {
                ...
                if ( $attempts > 1 ) {
                    print
                      "  ${YELLOW}Retry attempt $attempts/$max_attempts${NC}\n";
                }

                print
"  ${CYAN}Changing to $module_path to run terragrunt $operation${NC}\n";
                ...
                $terragrunt_cmd = "timeout $module->{timeout} $terragrunt_cmd"
                  if $module->timeout;
                ...
                if ( $result == 0 ) {
                    ...
                    if ( !$module->run_post_deploy($context) ) {
                        warn
                          "  ${YELLOW}Warning: Post-deploy hook failed${NC}\n";
                    }
            ...
            if ( !$success ) {
                ...
                if ( $module->critical && !$force ) {
                    die
"${RED}Critical module failed. Stopping deployment.${NC}\n";
                }
            }
        }
        else {
...
sub show_module_outputs {
    ...
    if ( !-f $output_file ) {
        ...
        print
          "This module may not have been deployed yet or has no outputs.\n\n";
...
sub capture_module_outputs {
    ...
    if ( $outputs && $outputs ne '' ) {
        ...
        if ( $@ || !$json_data ) {
            warn
"  ${YELLOW}Warning: Failed to parse terragrunt outputs for $module_path${NC}\n";
            print
"  ${YELLOW}Warning: Failed to parse terragrunt outputs for $module_path${NC}\n";
            return;
        }
        ...
        if ($@) {
            warn
"  ${YELLOW}Warning: Failed to write output.json for $module_path: $output_file: $@${NC}\n";
        }
    }
    else {
...
sub show_summary {
    ...
    if ( !$dry_run && $modules{_summary} ) {
        ...
        if (%execution_times) {
            ...
            my @sorted = sort { $execution_times{$b} <=> $execution_times{$a} }
        ...
        print
"${CYAN}Note: output.json files would be created after successful apply operations${NC}\n";
    }
...
sub extract_resource_type_from_path {
    ...
    return $type_mapping{$resource_name}
      if exists $type_mapping{$resource_name};
...
sub get_resource_category {
    ...
    my %categories = (
        compute =>
          [ 'eks', 'ecs', 'ec2', 'lambda', 'batch', 'lightsail', 'fargate' ],
        networking => [
            'vpc',              'security-group',
            'alb',              'nlb',
        ...
        monitoring =>
          [ 'cloudwatch', 'cloudtrail', 'sns', 'sqs', 'eventbridge', 'ssm' ],
        analytics =>
          [ 'opensearch', 'sagemaker', 'emr', 'glue', 'athena', 'kinesis' ],
